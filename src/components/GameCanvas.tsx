...
export function GameCanvas({ onGameEnd, courierType }: Props) {
  ...
  // ---- Spawn ----
  function spawnObject(state: typeof stateRef.current) {
    const wave = getLevelConfig(getLevel(state.score));
    const r = Math.random();
    const type: FallingObject['type'] =
      r < wave.goodProb ? 'good' :
  ...
  function addParticles(state: typeof stateRef.current, x: number, y: number, color: string, count = 8) {
    for (let i = 0; i < count; i++) {
      ...
      state.particles.push({
        ...
        alpha: 1, color, radius: 4 + Math.random() * 4,
      });
    }
  }
  ...
  function handleCatch(state: typeof stateRef.current, obj: FallingObject) {
    ...
    switch (obj.special) {
      case 'turbo':
        p.turboActive = true; p.turboTime = 300;
        addPopup(state, '⚡ ТУРБО!', LOGICAL_WIDTH / 2, 200, '#ffe082', 30); break;
      case 'magnet':
        p.magnetActive = true; p.magnetTime = 360;
        addPopup(state, '🧲 МАГНИТ!', LOGICAL_WIDTH / 2, 200, '#ce93d8', 30); break;
      case 'shield':
        p.shieldActive = true; p.shieldTime = 600;
        addPopup(state, '🛡️ ЩИТ!', LOGICAL_WIDTH / 2, 200, '#4fc3f7', 30); break;
      case 'energy':
        if (state.lives < 5) state.lives++;
        addPopup(state, '💚 +1 жизнь!', LOGICAL_WIDTH / 2, 200, '#69f0ae', 30); break;
      case 'combo':
        p.comboBoost = true; p.comboBoostTime = 300;
        addPopup(state, '🎯 КОМБО x2!', LOGICAL_WIDTH / 2, 200, '#ffd740', 30); break;
    }
  ...
  function drawBackground(ctx: CanvasRenderingContext2D, score: number, bgOffset: number) {
    ...
    if (bg && bg.complete) {
      // Three panels by level: 1-4=day, 5-8=night, 9+=rain
  ...
  function drawObject(ctx: CanvasRenderingContext2D, obj: FallingObject, t: number) {
    ...
    const glowColor = obj.type === 'good' ? 'rgba(100,255,130,0.35)' :
                      obj.type === 'neutral' ? 'rgba(255,210,50,0.35)' : 'rgba(255,60,60,0.35)';
  ...
  function drawPlayer(ctx: CanvasRenderingContext2D, player: Player) {
    ...
    if (player.shieldActive) {
      ...
      ctx.save();
      ctx.strokeStyle = `rgba(79,195,247,${0.5 + 0.4 * Math.sin(t * 4)})`;
    ...
    if (player.magnetActive) {
      ...
      ctx.lineWidth = 2; ctx.setLineDash([6, 6]); ctx.lineDashOffset = -t * 25;
    ...
    ctx.fillStyle = grad; ctx.fillRect(x - 70, y - 20, 140, 100);
    ctx.restore();
    ...
    if (drawSrc) {
      ctx.drawImage(drawSrc, x - W / 2, y - H / 2, W, H);
    } else {
  ...
  function drawHUD(ctx: CanvasRenderingContext2D, state: typeof stateRef.current) {
    ...
    const levelStartScore = (currentLevel - 1) * SCORE_PER_LEVEL;
    const levelEndScore   = currentLevel * SCORE_PER_LEVEL;
    const progress = Math.min((score - levelStartScore) / SCORE_PER_LEVEL, 1);
    const barW = 70, barH = 4, barX = LOGICAL_WIDTH / 2 - barW / 2, barY = 50;
    ...
    if (powers.length) {
      ctx.textAlign = 'left'; ctx.font = '16px serif'; ctx.textBaseline = 'top';
      powers.forEach((p, i) => ctx.fillText(p, 14 + i * 24, 36));
    }
  }
  ...
  function drawLevelBanner(ctx: CanvasRenderingContext2D, level: number, alpha: number) {
    ctx.save();
    ctx.globalAlpha = Math.min(alpha, 1);
    ctx.fillStyle = 'rgba(0,0,0,0.75)';
    ctx.fillRect(0, 68, LOGICAL_WIDTH, 46);
    ctx.font = 'bold 21px Nunito, sans-serif';
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillStyle = '#ffd740'; ctx.shadowColor = '#ffd740'; ctx.shadowBlur = 14;
    ctx.fillText(`⭐ УРОВЕНЬ ${level}! Сложнее!`, LOGICAL_WIDTH / 2, 91);
    ctx.restore();
  }

  // ---- Main effect: load assets + start loop ----
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    state.player = {
      ...
      comboBoost: false, comboBoostTime: 0,
    ...
    const courierSrc = courierType === 'male' ? '/courier-male.png' : '/courier-female.png';
      ...
      try {
        ...
        for (let i = 0; i < d.length; i += 4) {
          ...
          if (r > 235 && g > 235 && b > 235) {
            ...
            d[i+3] = Math.round(d[i+3] * (1 - (Math.min(r,g,b) - 215) / 20));
          }
        }
    ...
    try {
      ...
      engine.loop = true; engine.volume = 0.18;
    ...
    function loop(now: number) {
      ...
      if (state.spawnTimer <= 0) {
        spawnObject(state);
        state.spawnTimer = wave.spawnInterval + Math.random() * 28;
      }
      ...
      state.objects = state.objects.filter(obj => {
        ...
        if (p.magnetActive && obj.type !== 'danger') {
          ...
          if (dist < 230 && dist > 1) {
            obj.vx += (dx / dist) * 0.18 * dt;
            obj.vy += (dy / dist) * 0.18 * dt;
          }
        }

        if (obj.y > LOGICAL_HEIGHT + 60) {
          if (obj.type === 'good') state.combo = 0;
          return false;
        }
        if (circleCollide(p.x, p.y, p.radius, obj.x, obj.y, obj.radius)) {
          handleCatch(state, obj);
          return false;
        }
        return true;
      });
      ...
      state.particles = state.particles.filter(pt => {
        ...
        pt.vy += 0.09 * dt; pt.alpha -= 0.025 * dt;
        return pt.alpha > 0;
      });
      state.popups = state.popups.filter(pop => {
        pop.y += pop.vy * dt; pop.alpha -= 0.018 * dt;
        return pop.alpha > 0;
      });
      ...
      if (state.shakeTime > 0) {
        state.shakeTime -= dt;
        sx = (Math.random() - 0.5) * state.shakeTime * 1.2;
        sy = (Math.random() - 0.5) * state.shakeTime * 1.2;
      }
      ...
      drawObjects(ctx, state.objects, t);
      drawPlayer(ctx, state.player);
      drawPopups(ctx, state.popups);
      drawHUD(ctx, state);
      if (state.levelBannerTime > 0) {
        drawLevelBanner(ctx, state.currentLevel, state.levelBannerTime / 40);
      }
    ...
    // Input
    const onMouseMove   = (e: MouseEvent) => { stateRef.current.player.targetX = getLogicalX(e.clientX); };
    const onTouchMove   = (e: TouchEvent) => { e.preventDefault(); stateRef.current.player.targetX = getLogicalX(e.touches[0].clientX); };
    const onTouchStart  = (e: TouchEvent) => { e.preventDefault(); stateRef.current.player.targetX = getLogicalX(e.touches[0].clientX); };
  ...
  function drawObjects(ctx: CanvasRenderingContext2D, objects: FallingObject[], t: number) {
    for (const obj of objects) drawObject(ctx, obj, t);
  }
